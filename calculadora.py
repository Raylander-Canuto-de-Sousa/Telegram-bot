...
screen = Entry(root, text=inp, width=30,justify='right', font=(10), bd=4) 
...
for i in range(len(key_matrix)):   
    ...
    for j in range(len(key_matrix[i])):   
        btn_dict["btn_"+str(key_matrix[i][j])] = Button(root, bd=1, text=str(key_matrix[i][j]), font=myFont) 
        
        btn_dict["btn_"+str(key_matrix[i][j])].grid(row=i+1, column=j, padx=5, pady=5, ipadx=5, ipady=5) 
